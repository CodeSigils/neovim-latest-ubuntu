...
│       ├── check-author.yml      ← Enforces canonical human authorship and rejects agent trailers in new commits
...
- **Ubuntu 26.04-arm investigation pending** — GitHub has not yet released `ubuntu-26.04-arm` runner images. Track: https://github.com/actions/runner-images. When it arrives, update the repo-level variable `RUNNER_AARCH64` from `ubuntu-24.04-arm` to `ubuntu-26.04-arm` — all workflows pick it up without per-file changes (see docs/reproducibility.md §"Future: ubuntu-26.04 runner adoption").
...
**Build prerequisites** (Ubuntu 26.04 Resolute Raccoon): `ninja-build gettext cmake curl git build-essential` for manual/source builds. In this repo, `deps/ubuntu-build-deps.txt` is the source of truth for that host list, while CI/container-only extras live in `deps/ubuntu-ci-extra-deps.txt`.
...
- Build OS: Ubuntu LTS base image pinned in Containerfile
- Architectures: x86_64 and ARM64
- Build type: RelWithDebInfo
- Runtime dependencies: generated by CPack/dpkg-shlibdeps from the build container

## Prerequisites versions

Manual build prerequisites are listed in deps/ubuntu-build-deps.txt. Avoid hard-coded
compiler/runtime snapshots in README unless they are explicitly scoped to one verified build.
```
...
2. CI builds the Neovim `.deb` inside a container using `build.sh` for both `x86_64` (`${{ vars.RUNNER_X86_64 || 'ubuntu-latest' }}`) and `aarch64` (`${{ vars.RUNNER_AARCH64 || 'ubuntu-24.04-arm' }}`) in parallel; the container targets Ubuntu 26.04 regardless of runner OS
...
- Runner: `${{ vars.RUNNER_X86_64 || 'ubuntu-latest' }}` (x86_64) and `${{ vars.RUNNER_AARCH64 || 'ubuntu-24.04-arm' }}` (ARM64) via GitHub Actions matrix; container targets Ubuntu 26.04
...
**CI cycle efficiency**: The `paths-ignore` filter on `build.yml` skips pushes/PRs that only touch doc or metadata files (`*.md`, `LICENSE`, `docs/**`, `.mailmap`, `.gitignore`, `.gitattributes`, `.githooks/**`), non-build workflow config (`.github/dependabot.yml`), or workflow files (`nightly.yml`, `staleness.yml`, `check-author.yml`) that have their own independent triggers. Tag pushes always build fully because GitHub does not evaluate path filters for tag pushes.

### 9. Guardrails (Must Not Do)

#### 9.1 Agent Attribution (CI-Enforced — HARD BLOCK)

**Agents are tools, not contributors.** Authorship stays with the human maintainer (`CodeSigils <toolsoftrade.web@gmail.com>`). The [`check-author.yml`](.github/workflows/check-author.yml) workflow runs on every push/PR to `main` and enforces canonical maintainer authorship for human commits. It permits Dependabot commits and GitHub web/CLI merge commits only when the preserved author email is the canonical maintainer email. Outside those explicit exceptions, it hard-fails if any of these are detected:
...
Before staging any local commit, `git config user.name` and `git config user.email` MUST be set to the canonical maintainer identity. GitHub web/CLI merge commits are allowed only through the explicit workflow exception above, and bot commits are limited to Dependabot. The `.githooks/prepare-commit-msg` hook (activate via `git config core.hooksPath .githooks`) auto-strips known agent trailers as a safety net, but **do not rely on it** — the CI guard is the hard enforcement layer.
...
| 2026-05-23 | Release notes auto-generation | Added `generate_release_notes: true` to `softprops/action-gh-release@v3` step at the time; later superseded by a curated static release body with verified upstream links. |
...
| 2026-05-24 | Repo layout tree fix: add missing files | Added `.gitattributes` and then-current workflow files to the repository layout tree in AGENTS.md. Tree was stale — only listed `build.yml`. |
...
| 2026-05-24 | Removed "Debian" from README tagline | Project only tests on Ubuntu 26.04; claiming Debian support is inaccurate. RELEASING.md had no Debian references. Technical `.deb`/CPack references remain — they describe the packaging format, not a support commitment. |
...
||| 2026-07 | Runner labels externalized to repo-level variables | All 17 hardcoded `runs-on` labels across 7 workflow files replaced with `${{ vars.RUNNER_X86_64 || '...' }}` and `${{ vars.RUNNER_AARCH64 || '...' }}`. Updated AGENTS.md, docs/reproducibility.md. Set repo variables: `RUNNER_X86_64=ubuntu-latest`, `RUNNER_AARCH64=ubuntu-24.04-arm`. Nightly.yml now passes `--build-arg UBUNTU_VERSION`/`UBUNTU_CODENAME`/`UBUNTU_SHA256` for consistency with build.yml. Stale build.yml comment fixed. |
||| 2026-07 | UBUNTU_SHA256 externalized to repo variable | `Containerfile` now accepts `UBUNTU_SHA256` as a `--build-arg` (sourced from `vars.UBUNTU_SHA256`). The `FROM` line uses `ubuntu:${UBUNTU_VERSION}@sha256:${UBUNTU_SHA256}` — version, codename, and digest all changeable via one repo variable each instead of requiring a Containerfile edit. Both workflows pass all three `--build-arg` values. Set repo variable: `UBUNTU_SHA256=f3d28607...`. |
...
| C15 | Current Status §, docs/reproducibility.md §"Future: ubuntu-26.04 runner adoption" | ubuntu-26.04-arm runner availability tracked for future migration | Manual check: visit https://github.com/actions/runner-images — if `ubuntu-26.04-arm` listed, time to update the repo-level `RUNNER_AARCH64` variable |
...
9. **Investigation items (future runner adoption)** — ubuntu-26.04-arm pending in Current Status and docs/reproducibility.md — must be revisited to update the repo-level `RUNNER_AARCH64` variable when the runner is available
