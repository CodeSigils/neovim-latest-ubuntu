...
```bash
VERSION="latest"   # resolves to current stable via build.sh 'latest' alias
...
```bash
# Examples (matched to build.sh)
./build.sh                          # Build default version (from build.sh)
...
| **Lint**                        | 6 checks: `check-dependencies.py` (dep consistency), `check-labels.py` (required repo labels), `shellcheck build.sh test.sh scripts/*.sh`, `check-yaml-syntax.py` (YAML validation), `python3 -m unittest discover` (release readiness gate), `hadolint/hadolint-action@v3.4.0` (Containerfile lint — enforces Dockerfile/Containerfile best practices and catches security and efficiency issues in the build image definition) |
| **Verification**                | Exactly one expected architecture-specific `.deb`, matching Debian `Architecture`, before upload (fail-fast) per arch                                                                                                                                                                                                                                                                              |
...
| **Release aggregation**         | Separate `release` job runs only after the complete matrix succeeds, requires both named/architecture-verified packages, generates combined `SHA256SUMS`, then creates the Release with `softprops/action-gh-release@v3`                                                                                                                                                                           |
...
| ---------------- | ------------------------------------------------------------------------------------------------ | ------------------------------ |
| `VERSION`        | `latest` (resolves via GitHub API)                                                                     | Git tag — `v${VERSION}`        |
...
```bash
./build.sh                 # builds default version (from build.sh)
