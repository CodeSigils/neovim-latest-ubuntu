...
| **Trigger (branch push)** | `branches: [main]` with `paths-ignore: ['*.md', LICENSE, docs/**, .gitignore, .gitattributes, .githooks/**, .github/dependabot.yml, .github/workflows/staleness.yml, .github/workflows/check-author.yml]` — doc/metadata/workflow-edit pushes skip the build pipeline, but excluded workflows (staleness, author guard) still run via their own triggers |
...
2. **Main push** (non-doc changes) → CI builds matrix; doc/metadata/workflow-only pushes (`*.md`, `LICENSE`, `docs/**`, `.mailmap`, `.gitignore`, `.gitattributes`, `.githooks/**`, `.github/dependabot.yml`, `staleness.yml`, `check-author.yml`) skip the pipeline (each skipped workflow runs independently)
