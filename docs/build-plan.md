...
| **Trigger (branch push)** | `branches: [main]` with `paths-ignore: ['*.md', LICENSE, docs/**, .gitignore, .gitattributes, .githooks/**, .github/dependabot.yml, .github/workflows/nightly.yml, .github/workflows/staleness.yml, .github/workflows/check-author.yml]` — doc/metadata/workflow-edit pushes skip the build pipeline, but excluded workflows (staleness, author guard) still run via their own triggers |
