...
class FakeGitHub:
    ...
    def optional(self, path: str) -> dict | None:
        ...
        if self.existing:
            return {"draft": False, "assets": [{"name": name} for name in release_plan.CORE_ASSETS]}
        return None
...
class ReleasePlanTests(unittest.TestCase):
    ...
    def test_new_maintenance_release_builds_and_auto_publishes(self) -> None:
        plan = release_plan.make_plan(client=FakeGitHub(), mode="scheduled", requested="latest", publish_requested=False, repository="CodeSigils/neovim-latest-ubuntu")
    ...
    def test_feature_release_requires_environment_review(self) -> None:
        plan = release_plan.make_plan(client=FakeGitHub(latest="v0.13.0"), mode="scheduled", requested="latest", publish_requested=False, repository="CodeSigils/neovim-latest-ubuntu")
        self.assertTrue(plan.requires_review)

    def test_existing_complete_release_skips_expensive_build(self) -> None:
        plan = release_plan.make_plan(client=FakeGitHub(existing=True), mode="scheduled", requested="latest", publish_requested=False, repository="CodeSigils/neovim-latest-ubuntu")
        self.assertFalse(plan.should_build)
        self.assertIn("already published", plan.reason)

    def test_ci_build_never_publishes(self) -> None:
        plan = release_plan.make_plan(client=FakeGitHub(), mode="ci", requested="latest", publish_requested=False, repository="CodeSigils/neovim-latest-ubuntu")
        self.assertTrue(plan.should_build)
