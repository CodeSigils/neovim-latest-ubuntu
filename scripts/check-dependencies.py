...
def main() -> int:
    ...
    ensure("git clone" in build_sh_text, "FAIL: build.sh no longer uses git clone; revisit host build deps", errors)
    ensure("git" in build_set, "FAIL: git is required by build.sh but missing from ubuntu-build-deps.txt", errors)
    ensure("sudo " in test_sh_text, "FAIL: test.sh no longer uses sudo; revisit CI extra deps", errors)
    ensure("sudo" in ci_extra_set, "FAIL: sudo is required by test.sh but missing from ubuntu-ci-extra-deps.txt", errors)

    ensure("COPY deps/ /tmp/deps/" in container_text, "FAIL: Containerfile must copy deps/ manifests into the image", errors)
    ensure("ubuntu-build-deps.txt" in container_text, "FAIL: Containerfile must install deps/ubuntu-build-deps.txt", errors)
    ensure("ubuntu-ci-extra-deps.txt" in container_text, "FAIL: Containerfile must install deps/ubuntu-ci-extra-deps.txt", errors)

    overlap = sorted(build_set & ci_extra_set)
    ensure(not overlap, f"FAIL: dependency manifests overlap unexpectedly: {overlap}", errors)

    combined = build_set | ci_extra_set
    ensure(readme_set <= combined, "FAIL: README packages must be a subset of combined dependency manifests", errors)
